...
def generate_synthetic_data(N, M, L_0, b):
    ...
    for i in range(M):
        ...
        ranking_length = 100
        # ranking_length = np.random.randint(L_0 - 0.3*L_0 , L_0 + 0.3*L_0 )  # ランキングの長さ
...
def calculate_rankings(A, N):
    ...
    # 比率g_jに基づいてランキングを決定
    R_hat_indices = np.argsort(-g)  # 降順にソート
    R_hat = R_hat_indices + 1
...
for b in b_values:
    ...
    # Kendall Tau 距離の計算
    D = calculate_kendall_tau_distance(R_0 - 1, np.argsort(R_hat) + 1)
    D_values.append(D)
