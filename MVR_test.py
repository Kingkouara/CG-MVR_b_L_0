...
def iterative_constraint_relaxation(A):
    ...
    while True:
        ...
        #こうして推移率に従わなかった(i,j,k)に推移率を適用して再度LPさせる
        print("再実行LPの制約リスト A_ineq_matrix:")
        for i in range(len(A_ineq_matrix)):
            print(A_ineq_matrix[i])
        print("再実行LPの右辺値 b_ineq_vector:")
        print(b_ineq_vector)
...
if __name__ == "__main__":
    ...
    L_0 = 6  # ランキングの平均長さ
    # b = 0.5  # ノイズパラメータ
    b_values = [0.01, 0.5, 0.99]  # ノイズパラメータの候補
    D_values = []
